...
const SAMPLE_EVENTS: CalendarEvent[] = [
  ...
  { id: 'evt-5', title: '중간고사', date: format(addDays(new Date(), 3), 'yyyy-MM-dd'), startTime: '09:00', endTime: '11:00', categoryId: 'cat-5', memo: '경영학원론', userId: 'local', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
];

const SAMPLE_NOTES: Note[] = [
  { id: 'note-1', content: '다음 주 프레젠테이션 준비:\n- 슬라이드 30장 목표\n- 시연 영상 촬영\n- 발표 연습 3회 이상', userId: 'local', createdAt: addDays(new Date(), -1).toISOString(), updatedAt: addDays(new Date(), -1).toISOString() },
  { id: 'note-2', content: '읽고 싶은 책:\n1. 아침 루틴의 힘\n2. 습관의 힘\n3. 1만 시간의 재발견', userId: 'local', createdAt: addDays(new Date(), -3).toISOString(), updatedAt: addDays(new Date(), -3).toISOString() },
  { id: 'note-3', content: '장보기 목록:\n우유, 계란, 빵, 사과, 닭가슴살, 샐러드, 요거트', userId: 'local', createdAt: addDays(new Date(), -5).toISOString(), updatedAt: addDays(new Date(), -5).toISOString() },
];
...
export function PlannerProvider({ children }: { children: ReactNode }) {
  ...
  const [settings, setSettings] = useState<PlannerSettings>({ name: '', theme: 'slate', glassAccent: 'blue', glassAccentDark: 'blue', isDarkMode: false, groqApiKey: '' });
  const [todayBriefing, setTodayBriefing] = useState('좋은 하루입니다! 오늘 일정 5개가 등록되어 있습니다. 💪');
  ...
  const loadDataFromCloud = async (userId: string) => {
    ...
    try {
      ...
      if (eventsRes.data) {
        ...
        setEvents(cloudEvents.length > 0 ? cloudEvents : SAMPLE_EVENTS);
        localStorage.setItem('planner_events', JSON.stringify(cloudEvents.length > 0 ? cloudEvents : SAMPLE_EVENTS));
      }

      if (notesRes.data) {
        const cloudNotes = notesRes.data.map(n => ({ id: n.id, content: n.content, userId: n.user_id, createdAt: n.created_at, updatedAt: n.updated_at }));
        setNotes(cloudNotes.length > 0 ? cloudNotes : SAMPLE_NOTES);
        localStorage.setItem('planner_notes', JSON.stringify(cloudNotes.length > 0 ? cloudNotes : SAMPLE_NOTES));
      }

      if (categoriesRes.data) {
        const cloudCategories = categoriesRes.data.map(c => ({ id: c.id, name: c.name, color: c.color, emoji: c.emoji }));
      ...
      if (profileRes.data) {
        ...
        localStorage.setItem('planner_settings', JSON.stringify({ ...settings, ...profileSettings }));
      }
  ...
  }, [events, notes, categories, settings, studySessions, todayBriefing, isInitialized]);

  const saveToCloud = async (userId: string) => {
    try {
      await Promise.all([
        supabase.from('events').upsert(events.filter(e => e.userId === userId || e.userId === 'local').map(e => ({
          id: e.id, user_id: userId, title: e.title, date: e.date, start_time: e.startTime, end_time: e.endTime,
          category_id: e.categoryId, memo: e.memo, repeat: e.repeat, repeat_days: e.repeatDays,
        }))),
        supabase.from('notes').upsert(notes.filter(n => n.userId === userId || n.userId === 'local').map(n => ({
          id: n.id, user_id: userId, content: n.content,
        }))),
        supabase.from('categories').upsert(categories.map(c => ({ id: c.id, user_id: userId, name: c.name, color: c.color, emoji: c.emoji }))),
      ]);
    } catch (error) {
  ...
  const addEvent = async (eventData: Omit<CalendarEvent, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
    const newEvent: CalendarEvent = {
      ...eventData, id: crypto.randomUUID(), userId: user?.id || 'local',
      createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    };
    setEvents(current => [...current, newEvent]);
    if (user) {
      await supabase.from('events').insert({ id: newEvent.id, user_id: user.id, title: newEvent.title, date: newEvent.date, start_time: newEvent.startTime, end_time: newEvent.endTime, category_id: newEvent.categoryId, memo: newEvent.memo, repeat: newEvent.repeat, repeat_days: newEvent.repeatDays });
    }
  };

  const updateEvent = async (id: string, updates: Partial<CalendarEvent>) => {
    setEvents(current => current.map(evt => evt.id === id ? { ...evt, ...updates, updatedAt: new Date().toISOString() } : evt));
    if (user) {
      const dbUpdates: any = {};
      ...
      if (updates.repeat) dbUpdates.repeat = updates.repeat;
  ...
  const addNote = async (content: string) => {
    const newNote: Note = { id: crypto.randomUUID(), content, userId: user?.id || 'local', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    setNotes([...notes, newNote]);
    if (user) await supabase.from('notes').insert({ id: newNote.id, user_id: user.id, content: newNote.content });
  };

  const updateNote = async (id: string, content: string) => {
    setNotes(notes.map(note => note.id === id ? { ...note, content, updatedAt: new Date().toISOString() } : note));
    if (user) await supabase.from('notes').update({ content }).eq('id', id);
  };
  ...
  const addCategory = async (categoryData: Omit<Category, 'id'>) => {
    ...
    if (user) await supabase.from('categories').insert({ id: newCategory.id, user_id: user.id, name: newCategory.name, color: newCategory.color, emoji: newCategory.emoji });
  };
  ...
  const recordStudySession = (cardsStudied: number, correctAnswers: number) => {
    ...
    let newStreak = 1;
    if (lastSession && lastSession.date === yesterdayStr) newStreak = lastSession.streak + 1;
    else if (lastSession && lastSession.date === today) {
      setStudySessions(studySessions.map(session => session.date === today ? { ...session, cardsStudied: session.cardsStudied + cardsStudied, correctAnswers: session.correctAnswers + correctAnswers } : session));
      return;
    }
    setStudySessions([...studySessions, { date: today, cardsStudied, correctAnswers, streak: newStreak }]);
  };
  ...
  const refreshBriefing = async () => {
    if (!settings.groqApiKey) { setTodayBriefing('Groq API 키를 설정해주세요. 설정에서 API 키를 입력하세요.'); return; }
    try {
      const today = new Date().toISOString().split('T')[0];
      const todayEvents = events.filter(e => e.date === today);
      const upcomingEvents = events.filter(e => e.date >= today).sort((a, b) => a.date.localeCompare(b.date)).slice(0, 5).map(e => `${e.title}(${e.date})`);
  ...
  const value: PlannerContextType = { session, user, signInWithGoogle, signOut, syncState, manualSync, events, addEvent, updateEvent, deleteEvent, notes, addNote, updateNote, deleteNote, categories, addCategory, updateCategory, deleteCategory, flashCards, studySessions, recordStudySession, studyMode, setStudyMode, settings, updateSettings, todayBriefing, refreshBriefing };
