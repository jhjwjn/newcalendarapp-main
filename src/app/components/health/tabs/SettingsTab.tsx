...
import { Moon, Sun, Check, RefreshCw, LogIn, LogOut, Cloud, CloudOff, Key, Eye, EyeOff } from 'lucide-react';
...
export function SettingsTab({ theme }: SettingsTabProps) {
  ...
  return (
    <div className="mx-auto max-w-[720px] p-3 md:p-5 space-y-4">
      {/* 헤더 */}
      <div>
      ...
      <div className="rounded-2xl border p-4 md:p-5" style={{ background: theme.panelBackground, borderColor: theme.panelBorder }}>
        ...
        <div className="grid grid-cols-4 gap-3">
          {ACCENT_OPTIONS.map(opt => {
            ...
            return (
              <button key={opt.id} onClick={() => handleAccentChange(opt.id)} className="rounded-2xl p-3 text-center border transition-all" style={{ background: isSelected ? `${colors.primary}12` : theme.navBackground, borderColor: isSelected ? colors.primary : theme.line }}>
      ...
      <div className="rounded-2xl border p-4 md:p-5" style={{ background: theme.panelBackground, borderColor: theme.panelBorder }}>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="175" className="w-full rounded-2xl border px-4 py-3 text-sm outline-none" style={{ background: theme.navBackground, color: theme.text, borderColor: theme.line }} />
          </div>
          <div>
            <label className="text-xs font-semibold mb-1.5 block" style={{ color: theme.textMuted }}>세트 간 휴식 시간 (초)</label>
            <input type="number" value={restTime} onChange={e => setRestTime(e.target.value)} placeholder="90" className="w-full rounded-2xl border px-4 py-3 text-sm outline-none" style={{ background: theme.navBackground, color: theme.text, borderColor: theme.line }} />
          </div>
        </div>
      </div>
      ...
      {/* AI 설정 */}
      <div className="rounded-2xl border p-4 md:p-5" style={{ background: theme.panelBackground, borderColor: theme.panelBorder }}>
        <h2 className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: theme.textMuted }}>AI 설정</h2>
        <p className="text-xs mb-4" style={{ color: theme.textMuted }}>Groq API 키를 입력하면 AI 추천 기능을 사용할 수 있습니다.</p>
        <div className="flex items-center gap-2 rounded-2xl border px-4 py-3" style={{ background: theme.navBackground, borderColor: theme.line }}>
          <Key className="h-4 w-4 shrink-0" style={{ color: theme.textMuted }} />
          <input type={showKey ? 'text' : 'password'} value={groqKey} onChange={e => setGroqKey(e.target.value)} placeholder="gsk_..." className="flex-1 bg-transparent text-sm outline-none" style={{ color: theme.text }} />
          <button onClick={() => setShowKey(v => !v)}>{showKey ? <EyeOff className="h-4 w-4" style={{ color: theme.textMuted }} /> : <Eye className="h-4 w-4" style={{ color: theme.textMuted }} />}</button>
        </div>
      </div>
